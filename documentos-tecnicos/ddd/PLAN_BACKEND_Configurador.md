...
> **Diretório raiz:** `servicos-global/configurador/server/`
...
Execute em `servicos-global/configurador/server/` (e raiz do configurador):
...
Grep em `servicos-global/configurador/server/` por `z.object` para localizar todos os schemas afetados.
...
grep -r "prisma\.tenant\b" server/        # deve retornar zero
grep -r "prisma\.company\b" server/       # deve retornar zero
grep -r "prisma\.user\b" server/          # deve retornar zero (exceto configurador-db)
grep -r "DeployLog" server/               # deve retornar zero
grep -r "StripeEvent" server/             # deve retornar zero
grep -r "PriceTier" server/               # deve retornar zero
grep -r "TestPlan" server/                # deve retornar zero
```
