...
| `MASTER` | Todos os logs do **próprio tenant** (toda a organização) | `/workspace/historico-organizacao` |
| `STANDARD` | Apenas os logs onde **ele próprio** foi o ator | `/workspace/historico-organizacao` |
| `SUPPLIER` | Apenas os logs onde **ele próprio** foi o ator | `/workspace/historico-organizacao` |
...
1. Acessar `/workspace/historico-organizacao` → aba "Alertas" → botão "Nova Regra"
...
1. Ir em `/workspace/historico-organizacao` (MASTER) ou `/admin/historico-global` (SUPER_ADMIN/ADMIN)
...
1. Abrir o alerta em `/workspace/historico-organizacao` → aba "Alertas"
