...
**Composição SSOT** — `PARES_CASCADE_PEDIDO_ITEM` em `edicaoEmMassaService.ts` compõe **~61 pares**:

```
MAPA_PROPAGACAO_PEDIDO_ITEM (57 pares — shared/mapaPropagacaoPedidoItem.ts)
  + tipo_operacao_pedido → tipo_operacao_item           (exclusivo massa)
  + nome_exportador → nome_exportador_item              (JSON→coluna, exclusivo massa)
  + nome_importador → nome_importador_item              (idem)
  + nome_fabricante → nome_fabricante_item               (idem)
```

Os 57 pares SSOT cobrem: identidade comercial (5), casas decimais (4), câmbio (1), referências (3), datas pronto/inspeção/coleta (9), datas rascunho pedido (7), datas proforma (13), datas invoice (13), outras datas (2).
...
- **v4.0 (2026-05-14)** — Cascade SSOT + Incoterm dinâmico
  - `PARES_CASCADE_PEDIDO_ITEM` agora compõe SSOT (57 pares de `shared/mapaPropagacaoPedidoItem.ts`) + 4 exclusivos massa (~61 total)
  - Remove duplicação local de 25 pares — SSOT garante sincronia com create/patch
  - Incoterm: opcoes dinâmicas via `useIncotermsPedido()` (leitura ao vivo do Cadastros)
  - Remove 11 opcoes hardcoded de incoterm — respeita `cadastros-snapshot-policy`
  - Select desabilitado + banner quando incoterms indisponíveis (Mand. 08 — sem fallback silencioso)
