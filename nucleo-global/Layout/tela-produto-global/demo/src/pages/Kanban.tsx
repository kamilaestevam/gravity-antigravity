/**
 * Kanban.tsx — Demo do KanbanGlobal
 *
...
interface ItemKanbanDemo extends KanbanItem {
  nome:        string
  empresa:     string
  responsavel: string
  valor:       number
...
function gerarMock(): ItemKanbanDemo[] {
  ...
  return nomes.map((nome, i) => ({
    id:          `item-${i + 1}`,
    colunaKey:   colunaKeys[i % colunaKeys.length],
    nome,
...
export default function Kanban() {
  ...
  function handleMoverItem(itemId: string, novaColunaKey: string) {
    setItens(prev =>
      prev.map(item =>
        item.id === itemId ? { ...item, colunaKey: novaColunaKey } : item,
      ),
    )
  }
