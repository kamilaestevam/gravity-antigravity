...
export const SelectColunasGlobal = memo(function SelectColunasGlobal({
  ...
  // ── Visíveis em ordem (espelha a tabela) ──────────────────────────────────
  const colunasVisivelOrdenadas = useMemo(
    () => colunasVisiveis
  ...
  // ── Ocultas agrupadas ─────────────────────────────────────────────────────
  const gruposOcultos = useMemo<GrupoInterno[]>(() => {
    const ocultas = colunas.filter(c => !colunasVisiveis.includes(c.key))
    const mapa = new Map<string, ColunaSelectConfig[]>()
    for (const col of ocultas) {
      const nome = col.grupo ?? GRUPO_GERAL
      if (!mapa.has(nome)) mapa.set(nome, [])
      mapa.get(nome)!.push(col)
    }
    return Array.from(mapa.entries()).map(([nome, cols]) => ({ nome, colunas: cols }))
  }, [colunas, colunasVisiveis])
  ...
  // grupoNome === '__visible__' identifica itens da seção visível (drag livre entre si)
  function renderItem(col: ColunaSelectConfig, idx: number, lista: ColunaSelectConfig[], grupoNome?: string) {
    ...
    const ehSecaoVisivel = grupoNome === '__visible__'

    return (
      <React.Fragment key={col.key}>
        {/* Divisor entre obrigatórias e opcionais (apenas na lista plana de busca) */}
        {!grupoNome && !col.naoOcultavel && prevObrigatorio && (
          <div className="scg-divisor" />
        )}
        <label
          className={`scg-item${col.naoOcultavel ? ' scg-item--locked' : ''}`}
          draggable={!!onReordenar && !col.naoOcultavel && ehSecaoVisivel}
          onDragStart={() => {
        ...
        >
          {/* Cadeado ou handle de drag — apenas na seção visível */}
          {onReordenar && ehSecaoVisivel && (
    ...
    >
      ...
      <div className="scg-lista">
        {estaFiltrando ? (
          ...
          /* Modo normal: duas seções — Visíveis (flat+drag) e Ocultas (por grupo) */
          <>
            {/* Seção: Visíveis */}
            <div className="scg-secao-header">
              Visíveis
              <span className="scg-grupo-badge">{colunasVisivelOrdenadas.length}</span>
            </div>
            {colunasVisivelOrdenadas.length === 0 ? (
              <div className="scg-vazio">Nenhuma coluna visível</div>
            ) : (
              colunasVisivelOrdenadas.map((col, idx) =>
                renderItem(col, idx, colunasVisivelOrdenadas, '__visible__')
              )
            )}
            ...
            {gruposOcultos.length > 0 && (
              <>
                <div className="scg-secao-header">
                  Ocultas
                  <span className="scg-grupo-badge">
                    {colunas.length - colunasVisivelOrdenadas.length}
                  </span>
                </div>
                {gruposOcultos.map(grupo => renderGrupo(grupo))}
              </>
            )}
          </>
        )}
