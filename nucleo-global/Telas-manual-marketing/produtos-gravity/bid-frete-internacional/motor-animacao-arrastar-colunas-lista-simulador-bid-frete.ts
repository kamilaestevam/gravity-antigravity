...
import type { LadoDropColuna } from './reordenar-colunas-lista-simulador-bid-frete'
...
export const SEQUENCIA_DEMO_ARRASTAR_COLUNAS_LISTA_BID_FRETE: PassoAnimacaoArrastarColunaLista[] = [
  { tipo: 'espera', duracaoMs: 900 },
  {
    tipo: 'mover',
    origemId: 'modalidade',
    destinoId: 'porto_destino',
    lado: 'after',
    duracaoMs: 1400,
    soltarEm: 0.82,
  },
  { tipo: 'espera', duracaoMs: 500 },
  {
    tipo: 'mover',
    origemId: 'status',
    destinoId: 'operacao',
...
function garantirCabecalhoVisivelNoContainer(container: HTMLElement, colunaId: string) {
  const cabecalho = container.querySelector<HTMLElement>(`[data-coluna-th-demo-id="${colunaId}"]`)
  const areaRolagem = container.querySelector<HTMLElement>('.bfs-lista-tabela-wrap')
...
export function iniciarMotorAnimacaoArrastarColunasListaBidFrete({
  ativo,
  containerRef,
  aoMoverColuna,
  aoArrastarColuna,
  ...
  const executarLoop = async () => {
    while (!cancelado) {
      ...
      for (const passo of SEQUENCIA_DEMO_ARRASTAR_COLUNAS_LISTA_BID_FRETE) {
        if (cancelado) break
