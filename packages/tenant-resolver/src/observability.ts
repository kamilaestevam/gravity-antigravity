  ...
  _logger = {
    error(obj, msg) {
      process.stderr.write(
        JSON.stringify({ level: 'error', service: '@gravity/tenant-resolver', msg, ...obj }) + '\n',
      );
    },
    warn(obj, msg) {
      if (!isTest) {
        process.stderr.write(
          JSON.stringify({ level: 'warn', service: '@gravity/tenant-resolver', msg, ...obj }) + '\n',
        );
      }
    },
    info(obj, msg) {
      if (!isTest) {
        process.stderr.write(
          JSON.stringify({ level: 'info', service: '@gravity/tenant-resolver', msg, ...obj }) + '\n',
        );
      }
    },
  };
