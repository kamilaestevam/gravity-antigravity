...
describe('Contrato de resposta — TransferPreview', () => {
  it('estrutura de preview tem todos os campos obrigatórios', () => {
    ...
    expect(preview.origem).toHaveProperty('quantidade_saldo_pedido')
  ...
  it('quantidade_saldo_pedido é número (não string/Decimal)', () => {
    expect(typeof PREVIEW_RESULT.origem.quantidade_saldo_pedido).toBe('number')
...
describe('Fluxo completo de transferência — reducao_simples', () => {
  it('passo 1: preview calcula corretamente 111 - 110 = 1', async () => {
    ...
    mockPreview.mockResolvedValue({
      ...PREVIEW_RESULT,
      origem: { ...PREVIEW_RESULT.origem, quantidade_saldo_pedido: 111, quantidade_apos: 1 },
    })
    const service = new (TransferirService as any)()
