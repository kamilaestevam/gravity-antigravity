/**
 * Upsert de portos ausentes — leva Oceania / Oriente Médio / territórios.
 * Uso: npx tsx scripts/sob-demanda/upsert-portos-faltantes-leva-oceania-oriente.ts
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { PrismaClient } from '../../servicos-global/cadastros/generated/index.js'
import { PORTOS_UPSERT_LEVA_OCEANIA_ORIENTE } from './data/portos-upsert-leva-oceania-oriente.js'
...
async function main() {
  let upserted = 0
  for (const p of PORTOS_UPSERT_LEVA_OCEANIA_ORIENTE) {
  ...
  console.log(`\n[upsert-portos-oceania-oriente] ${upserted} portos upserted (ativo_porto=true)`)
}
