/**
 * Verifica portos da última leva (Oceania / Oriente Médio / territórios) contra cadastros.porto.
 * Uso: npx tsx scripts/sob-demanda/verificar-portos-leva-oceania-oriente-cadastros.ts
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { PrismaClient } from '../../servicos-global/cadastros/generated/index.js'
import { PORTOS_CANONICOS_LEVA_OCEANIA_ORIENTE } from './data/portos-canonicos-leva-oceania-oriente.js'
import { PORTOS_LEVA_OCEANIA_ORIENTE } from './data/portos-leva-oceania-oriente.js'
...
async function main() {
  ...
  for (const item of PORTOS_LEVA_OCEANIA_ORIENTE) {
    porPais[item.pais] ??= { ok: 0, amb: 0, miss: 0 }
    const codigoCanonico = PORTOS_CANONICOS_LEVA_OCEANIA_ORIENTE[`${item.pais}:${item.rotulo}`]
    if (codigoCanonico) {
  ...
  console.log(`\n=== LEVA OCEANIA/ORIENTE — ${PORTOS_LEVA_OCEANIA_ORIENTE.length} portos ===`)
