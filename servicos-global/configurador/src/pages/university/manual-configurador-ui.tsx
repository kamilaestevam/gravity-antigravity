...
import {
  ...
  Plus, MinusCircle, Warning, FunnelSimple, CubeTransparent, TextT,
  List, SquaresFour, ChartBar, ListChecks,
...
import { ManualInfograficoBidFreteControlesMapa } from './manual-bid-frete-infografico-controles-mapa'
...
function ManualGaleriaTelaParagrafoFigura({
  ...
  return (
    <div style={{
      marginBottom: entreLinhas ? MANUAL_ESPACO_PARAGRAFO_PX : MANUAL_ESPACO_ANTES_IMAGEM_ACORDEAO_PX,
      textAlign: 'left',
  ...
  if (modoTituloSubtopico) {
    ...
    return (
      <div style={{
        ...
        <ManualParagrafo texto={textoTitulo} marginBottom={0} />
      </div>
    )
  }
...
function ManualGaleriaTelaCelula({ tela }: { tela: DocGaleriaTela }) {
  ...
  const temPilares = Boolean(
    pilaresCustomizacao?.length || pilaresMapaBidFrete?.length || pilaresFiltrosMapaBidFrete?.length,
  )
...
function ManualPassosSubtopicosAcordeao({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: MANUAL_ESPACO_PARAGRAFO_PX, marginTop: MANUAL_ESPACO_ENTRE_PASSOS_PX }}>
      {passos.map((passo) => {
    ...
    <div style={{
      ...
      {passo.paragrafos?.map((p, i) => {
        ...
        <div key={i}>
          ...
          {passo.mostrarInfograficoBidFreteControlesMapa && i === 0 ? (
  ...
  const galeriaAposTabela = passo.galeriaTelasAposTabela?.length ? (
    <div style={{ marginTop: MANUAL_ESPACO_ENTRE_PASSOS_PX }}>
      {passo.galeriaTelasAposTabela.map((tela, indiceGaleria) => (
        <div
          key={tela.legenda}
          style={{ marginTop: indiceGaleria === 0 ? 0 : MANUAL_ESPACO_ENTRE_PASSOS_PX }}
        >
          <ManualGaleriaTelaCelula tela={tela} />
        </div>
      ))}
...
function ManualGaleriaComparacaoIntro({
  ...
  const espacoAcimaGaleriaPx = emAcordeaoSubtopico
    ? MANUAL_ESPACO_ANTES_IMAGEM_ACORDEAO_PX
    : MANUAL_ESPACO_PARAGRAFO_PX
  const espacoLegendaPrintFiguraPx = textoAcimaEstiloCorpo || emAcordeaoSubtopico
    ? MANUAL_ESPACO_ANTES_IMAGEM_ACORDEAO_PX
    : MANUAL_ESPACO_PARAGRAFO_PX
  ...
  const renderTela = (
    ...
    <div
      ...
      style={{
        ...
        ...((textoAcimaEstiloCorpo && colunasGrade > 1 && !printLarguraTotal)
    ...
    >
      ...
      <ManualFiguraScreenshot
        ...
        larguraTotal={printLarguraTotal}
      />
    </div>
  )
  ...
  return (
    <div style={{
      margin: textoAcimaEstiloCorpo
        ...
        : `${MANUAL_ESPACO_ENTRE_PASSOS_PX}px 0 ${MANUAL_ESPACO_ENTRE_PASSOS_PX}px`,
      ...
      {tituloEtapa && mostrarChipsTransferirTresTipos ? (
        ...
        <div style={{
          ...
          <ManualChipBidFreteModalTransporte id={chipBidFreteModalTransporte} />
          <ManualParagrafo texto={tituloEtapa} marginBottom={0} />
        </div>
        ...
        <div style={{
          ...
          <ManualChipBidFreteTipoCarga id={chipBidFreteTipoCarga} />
          <ManualParagrafo texto={tituloEtapa} marginBottom={0} />
        </div>
