 ...
 * Header esperado: `x-internal-key` com o valor de `process.env.INTERNAL_SERVICE_KEY`.
 *
...
export function requireInternalKey(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!expected) {
    console.error('[cadastros][internal-key] INTERNAL_SERVICE_KEY não definida no ambiente')
