...
import { useDashboardLayout, type GridLayoutItem } from '../hooks/useDashboardLayout.js'

import type { DashboardWidgetConfig, DashboardConfig } from '../store/dashboardStore.js'
    ...
    <div style={{
      ...
      <div>
        <p style={{ color: 'var(--text-primary, #f1f5f9)', fontWeight: 600, margin: '0 0 0.5rem', fontSize: '1rem' }}>
          Dashboard ainda sem widgets
        </p>
        <p style={{ color: 'var(--text-muted, #64748b)', fontSize: '0.875rem', margin: 0 }}>
          Adicione widgets para visualizar KPIs, gráficos e métricas do seu negócio.
        </p>
      </div>
      <button
        onClick={onAddWidget}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
            ...
            <input
              ...
              placeholder="Ex: Total de Operações"
              style={{
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
            ...
            <input
              ...
              placeholder="Ex: saving_total, valor_operado"
              style={{
                padding: '0.5rem 0.75rem',
                borderRadius: 8,
                border: '1px solid var(--bg-elevated, #475569)',
...
export function DashboardGeralPage() {
  ...
  const { connected } = useDashboardSSE({
    ...
    onWidgetUpdate: (widgetId) => {
      // Força refetch do widget afetado via evento customizado
  ...
  // Layouts é Record<string, Layout[]> do react-grid-layout — compatível com GridLayoutItem
    ...
    return (
      <div style={{ padding: '1.5rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div style={{
          ...
          {Array.from({ length: 6 }).map((_, i) => <SkeletonWidget key={i} />)}
        </div>
    ...
    return (
      <div style={{ padding: '1.5rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div style={{
          ...
          <WarningCircle size={32} style={{ color: 'var(--danger, #ef4444)', marginBottom: '0.75rem' }} />
  ...
  // ─── Criar config GENERAL se não existir ──────────────────────────────────
