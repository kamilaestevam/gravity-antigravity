...
async function logSecurityEvent(event: SecurityEventInput): Promise<void> {
  ...
  await AuditService.log({
    ...
    status: event.metadata?.blocked ? EventoStatus.FALHA : EventoStatus.SUCESSO,
  ...
  // A rota /api/v1/admin/eventos-seguranca (admin) está atrás de requireAuth+
...
export const securityAudit = {
  ...
  rateLimitHit(tenantId: string, details: {
    ...
    return logSecurityEvent({
      tenant_id: tenantId,
      actor_id: 'system',
      actor_type: AcaoExecutadaPor.INTEGRACAO,
  ...
  apiKeyUsed(tenantId: string, apiKeyId: string, details: {
    ...
    return logSecurityEvent({
      tenant_id: tenantId,
      actor_id: apiKeyId,
      actor_type: AcaoExecutadaPor.API,
      action: 'API_CALL',
