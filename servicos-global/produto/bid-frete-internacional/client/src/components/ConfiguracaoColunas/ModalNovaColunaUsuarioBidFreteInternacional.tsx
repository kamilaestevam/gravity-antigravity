...
import {
  parsearFormula,
  detectarCircular,
} from '../../shared/formula-engine-coluna-bid-frete-internacional'
import {
  FORMULA_ALIAS_MAP,
  formulaParaAlias,
  formulaParaChave,
...
} from '../../shared/formula-utils-coluna-bid-frete-internacional'
import { analisarSemanticaFormula, SEMANTICA_CAMPOS } from '../../shared/gabi-semantica-formula-bid-frete-internacional'
import './ModalNovaColunaBidFreteInternacional.css'
...
const ESCOPO_COLUNA_COTACAO_BID_FRETE: EscopoColunaUsuario = 'pedido'
...
interface ModalNovaColunaProps {
  ...
  onFechar: () => void
  onSalvar: (payload: PayloadSalvarColunaUsuarioBidFreteInternacional & { id?: string; tipo: TipoColunaUsuario }) => void | Promise<void>
...
export function ModalNovaColunaUsuarioBidFreteInternacional({
  colunaEdicao,
  onFechar,
  onSalvar,
  camposDisponiveis = [],
  ...
  const handleSalvar = useCallback(async () => {
    ...
    const basePayload: PayloadSalvarColunaUsuarioBidFreteInternacional & { id?: string; tipo: TipoColunaUsuario } = {
      ...(isEdicao && colunaEdicao ? { id: colunaEdicao.id } : {}),
      nome: nomeTrimmed,
      tipo: isEdicao && colunaEdicao ? colunaEdicao.tipo : tipo,
      escopo: ESCOPO_COLUNA_COTACAO_BID_FRETE,
      visibilidade,
      obrigatorio: false,
      alerta_divergencia_itens: false,
      valor_padrao: tipoFormula ? formulaChave : '',
      formula_expressao: tipoFormula ? formulaChave : undefined,
      descricao: descricao.trim(),
      opcoes: tipoComOpcoes ? opcoes : [],
      ativo: colunaEdicao?.ativo ?? true,
    }

    try {
      await Promise.resolve(onSalvar(basePayload))
      onFechar()
    } catch (err) {
    ...
    formulaErro, isEdicao, colunaEdicao, onSalvar, onFechar, definirErro, limparErro, t,
  ])
  ...
  return (
    <ModalFormularioAbasGlobal
      ...
      subtitulo={isEdicao
        ? t('pedido.modal_col.subtitulo_edicao')
        : t('bidfrete.modal_col.subtitulo_novo', { defaultValue: 'Crie uma coluna personalizada para as cotações de frete.' })}
      tamanho="md"
      larguraMaxima="560px"
      semAbas
      dirty
