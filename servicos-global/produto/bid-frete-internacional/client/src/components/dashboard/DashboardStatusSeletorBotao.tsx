...
export function DashboardStatusSeletorBotao({
  ...
  const painel = open && typeof document !== 'undefined'
    ? createPortal(
        <div
          ref={painelRef}
          className="bid-frete-dashboard-toolbar-icon-dropdown__painel bid-frete-dashboard-toolbar-icon-dropdown__painel--portal"
        ...
        >
          <button
            ...
            className={`bid-frete-dashboard-toolbar-icon-dropdown__opcao bid-frete-dashboard-toolbar-icon-dropdown__opcao--multi${isTodosActive ? ' bid-frete-dashboard-toolbar-icon-dropdown__opcao--ativa' : ''}`}
            data-testid="status-chip-todos"
            onClick={() => onStatusChange([])}
          >
            <span className="bid-frete-dashboard-toolbar-icon-dropdown__checkbox" aria-hidden="true">
              {isTodosActive ? <Check size={12} weight="bold" /> : <Square size={12} />}
            </span>
            <span className="bid-frete-dashboard-toolbar-icon-dropdown__rotulo">
              {t('nucleo.dashboard.barra.todos')}
            </span>
            {statusCounts !== undefined && (
              <span className="bid-frete-dashboard-toolbar-icon-dropdown__contagem">
                ({statusCounts['todos'] ?? Object.values(statusCounts).reduce((a, b) => a + b, 0)})
              </span>
            )}
          </button>

          <div className="bid-frete-dashboard-toolbar-icon-dropdown__separador" role="separator" />
          ...
          {statusOptions.map(opt => {
            ...
            return (
              <button
                ...
                className={`bid-frete-dashboard-toolbar-icon-dropdown__opcao bid-frete-dashboard-toolbar-icon-dropdown__opcao--multi${active ? ' bid-frete-dashboard-toolbar-icon-dropdown__opcao--ativa' : ''}${isDisabled ? ' bid-frete-dashboard-toolbar-icon-dropdown__opcao--desabilitada' : ''}`}
                style={active && customColors ? { color: customColors.text } : undefined}
                data-testid={`status-chip-${opt}`}
                onClick={() => toggleStatus(opt)}
              >
                <span
                  className="bid-frete-dashboard-toolbar-icon-dropdown__checkbox"
                  aria-hidden="true"
                  style={active && customColors ? { color: customColors.text } : undefined}
                >
                ...
                {statusCounts !== undefined && count !== undefined && (
                  <span className="bid-frete-dashboard-toolbar-icon-dropdown__contagem">({count})</span>
                )}
              </button>
            )
          })}
  ...
  return (
    <div ref={ref} className="bid-frete-dashboard-toolbar-icon-dropdown">
