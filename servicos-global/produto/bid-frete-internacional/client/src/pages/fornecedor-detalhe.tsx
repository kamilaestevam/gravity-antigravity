...
const moeda_ganho_bid_frete_internacional = (val: number, currency: string) =>
...
export default function DetalheFornecedor() {
  ...
  const colunasPrecos: TabelaGlobalColuna<any>[] = [
    {
      key: 'origem_nome_cotacao_bid_frete_internacional',
      label: t('bidfrete.detalhe_fornecedor.col_rota'),
      tipo: 'texto',
      largura: 240,
      render: (_val: unknown, item: TabelaPreco) => (
        <span style={{ fontSize: '0.8125rem', color: 'var(--text-primary, #f1f5f9)' }}>
          {item.origem_nome_cotacao_bid_frete_internacional} <span style={{ color: 'var(--text-muted)' }}>→</span> {item.destino_nome_cotacao_bid_frete_internacional}
        </span>
      ),
    },
    {
      ...
      render: (val: unknown) => {
        ...
        return MODAL_LABELS[modal] ?? modal
      },
    },
    {
      ...
      largura: 100,
      render: (val: unknown) => {
        const modalidade = val as ModalidadeCarga
    ...
    },
    {
      ...
      largura: 110,
      align: 'right',
      render: (val: unknown, item: TabelaPreco) => (
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.8125rem' }}>
          {moeda_ganho_bid_frete_internacional(val as number, item.moeda_ganho_bid_frete_internacional)}
        </span>
      ),
    },
    {
      ...
      render: (_val: unknown, item: TabelaPreco) => (
        <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '0.8125rem', color: 'var(--text-secondary, #94a3b8)' }}>
          {moeda_ganho_bid_frete_internacional(item.taxas_origem_proposta_bid_frete_internacional + item.taxas_destino_proposta_bid_frete_internacional, item.moeda_ganho_bid_frete_internacional)}
        </span>
      ),
    },
    {
      ...
      align: 'right',
      render: (val: unknown) => (
        <span style={{ fontSize: '0.8125rem' }}>{val as number} {t('bidfrete.detalhe_cotacao.dias')}</span>
      ),
    },
    {
      key: 'validade_fim',
      ...
      tipo: 'periodo',
      largura: 110,
      render: (val: unknown) => dataBR(val as string),
    },
  ]
  ...
  return (
    <PaginaGlobal className="bf-detalhe-forn">
        ...
        <>
          ...
          <div className="bf-det-metrics">
            <div className="bf-det-metric">
              ...
              <div className="bf-det-metric-label">{t('bidfrete.detalhe_fornecedor.nota_global_classificacao_bid_frete_internacional')}</div>
            ...
            {tab === 'info' && (
              <div className="bf-det-info-grid">
                ...
                <InfoField
                  ...
                  value={fornecedor.aceita_cotacao_aberta_fornecedor_bid_frete_internacional ? t('bidfrete.detalhe_fornecedor.sim') : t('bidfrete.detalhe_fornecedor.nao')}
                />
                <InfoField
                    ...
                    <div key={av.id} className="bf-det-avaliacao-card">
                      ...
                      <div className="bf-det-avaliacao-cats">
                        <div className="bf-det-avaliacao-cat">
                          ...
                          <Stars rating={av.nota_frete_avaliacao_bid_frete_internacional} size={12} />
                        </div>
                        <div className="bf-det-avaliacao-cat">
                          <span className="bf-det-avaliacao-cat-label">{t('bidfrete.detalhe_fornecedor.cat_atendimento')}</span>
                          <Stars rating={av.nota_atendimento_avaliacao_bid_frete_internacional} size={12} />
                        </div>
                        <div className="bf-det-avaliacao-cat">
                        ...
                        <div className="bf-det-avaliacao-cat">
                          ...
                          <Stars rating={av.nota_confiabilidade_avaliacao_bid_frete_internacional} size={12} />
                        </div>
                      </div>
                      {av.comentario_avaliacao_bid_frete_internacional && (
                        <div className="bf-det-avaliacao-comentario_avaliacao_bid_frete_internacional">
                          {av.comentario_avaliacao_bid_frete_internacional}
                        </div>
                      )}
                    </div>
      ...
      <style>{`
        ...
        .bf-det-avaliacao-comentario_avaliacao_bid_frete_internacional {
          font-size: 0.8125rem;
          color: var(--text-secondary, #94a3b8);
          line-height: 1.5;
          padding-top: 0.75rem;
