/**
 * Exportação padrão da lista BID Frete (6 formatos — paridade Lista de Pedidos).
 */
...
export interface MontarAcoesExportacaoListaBidFreteParams<T> {
...
function montarDadosExport<T>(
  ...
  return dados.map(row => {
    ...
    for (const col of colunasExport) {
      linha[col.key] = formatValorExport(col.key, row)
    }
    return linha
  })
}

export function montarAcoesExportacaoListaBidFrete<T>(
  params: MontarAcoesExportacaoListaBidFreteParams<T>,
): GTAcaoExport[] {
  const {
    colunas,
    preferencias,
