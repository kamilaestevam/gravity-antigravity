...
export const BUILT_IN_DERIVED = [
    {
        ...
        operation: 'RATIO',
        formula: ({ saving_total, cotacoes_passadas }) => cotacoes_passadas > 0 ? saving_total / cotacoes_passadas : null,
        fieldType: 'currency',
    },
    {
    ...
    },
    {
        ...
        operation: 'RATIO',
        formula: ({ valor_aprovado_usd, cotacoes_passadas }) => cotacoes_passadas > 0 ? valor_aprovado_usd / cotacoes_passadas : null,
        fieldType: 'currency',
    },
];
export function computeDerived(metric, values) {
    try {
...
export function loadUserDerivedMetrics() {
    try {
...
export function saveUserDerivedMetrics(metrics) {
    localStorage.setItem(LS_KEY, JSON.stringify(metrics));
}
export function getAllDerivedMetrics() {
    return [...BUILT_IN_DERIVED, ...loadUserDerivedMetrics()];
}
//# sourceMappingURL=derivedMetrics.js.map