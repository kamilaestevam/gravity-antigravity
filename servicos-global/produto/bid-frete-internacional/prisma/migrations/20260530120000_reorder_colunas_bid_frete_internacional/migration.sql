-- Reordena colunas físicas (Railway UI) — id_workspace após id_organizacao;
-- cotacao: id_bid na 2ª coluna; proposta: id_proposta, id_cotacao, id_bid no topo.
-- Idempotente: pula se a 2ª coluna já estiver correta.

-- ─── Snapshot id_bid na proposta ───────────────────────────────────────────────
ALTER TABLE "proposta_bid_frete_internacional"
  ADD COLUMN IF NOT EXISTS "id_bid_bid_frete_internacional" TEXT;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'proposta_bid_frete_internacional_id_bid_bid_frete_internacional_fkey'
  ) THEN
    ALTER TABLE "proposta_bid_frete_internacional"
...
-- ─── Helper: recria tabela com ordem de colunas desejada ─────────────────────
...
BEGIN
  ...
  IF v_second_col = p_column_order[2] THEN
    RETURN;
  END IF;

  -- FKs que referenciam ou saem desta tabela
  FOR v_rec IN
    SELECT
  ...
  FOR i IN 1..array_length(p_column_order, 1) LOOP
    v_col := p_column_order[i];
  ...
  IF v_pk_cols IS NOT NULL THEN
    v_create_parts := array_append(
      ...
      format(
        'PRIMARY KEY (%s)',
        array_to_string(
          ARRAY(SELECT format('%I', col) FROM unnest(v_pk_cols) AS col),
          ', '
        )
      )
    );
  ...
  EXECUTE format('CREATE TABLE %I (%s)', v_tmp, array_to_string(v_create_parts, ', '));
  EXECUTE format(
...
-- ─── Recria FKs e índices essenciais (dropados no reorder) ───────────────────
DO $$ BEGIN
  IF NOT EXISTS (
...
CREATE INDEX IF NOT EXISTS "bid_frete_internacional_id_organizacao_id_produto_gravity_idx"
  ON "bid_frete_internacional"("id_organizacao", "id_produto_gravity");
CREATE INDEX IF NOT EXISTS "bid_frete_internacional_id_organizacao_id_usuario_idx"
  ON "bid_frete_internacional"("id_organizacao", "id_usuario");
...
CREATE INDEX IF NOT EXISTS "bid_frete_internacional_id_organizacao_status_bid_bid_frete_internacional_idx"
  ON "bid_frete_internacional"("id_organizacao", "status_bid_bid_frete_internacional");
CREATE INDEX IF NOT EXISTS "bid_frete_internacional_numero_bid_bid_frete_internacional_idx"
  ON "bid_frete_internacional"("numero_bid_bid_frete_internacional");
