-- Reordena colunas físicas da cotação — fluxo LOC/DOC (Railway UI).
-- Rascunho: identificação → operação/modal → status/visibilidade → rota → carga → mercadoria → termos → meta/prazos → datas.
-- Idempotente: bfi_reorder_table_columns compara ordinal_position.

-- PG trunca identificadores > 63 chars — renomeia colunas estado_provincia_* rodoviario
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'cotacao_bid_frete_internacional'
      AND column_name = 'estado_provincia_origem_rodoviario_cotacao_bid_frete_internacio'
  ) THEN
    ALTER TABLE "cotacao_bid_frete_internacional"
      RENAME COLUMN "estado_provincia_origem_rodoviario_cotacao_bid_frete_internacio"
      TO "estado_provincia_origem_rodov_cotacao_bid_frete_internacional";
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'cotacao_bid_frete_internacional'
      AND column_name = 'estado_provincia_origem_rodoviario_cotacao_bid_frete_internacional'
  ) THEN
    ALTER TABLE "cotacao_bid_frete_internacional"
      RENAME COLUMN "estado_provincia_origem_rodoviario_cotacao_bid_frete_internacional"
      TO "estado_provincia_origem_rodov_cotacao_bid_frete_internacional";
  END IF;
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'cotacao_bid_frete_internacional'
      AND column_name = 'estado_provincia_destino_rodoviario_cotacao_bid_frete_internaci'
  ) THEN
    ALTER TABLE "cotacao_bid_frete_internacional"
      RENAME COLUMN "estado_provincia_destino_rodoviario_cotacao_bid_frete_internaci"
      TO "estado_provincia_destino_rodov_cotacao_bid_frete_internacional";
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'cotacao_bid_frete_internacional'
      AND column_name = 'estado_provincia_destino_rodoviario_cotacao_bid_frete_internacional'
  ) THEN
    ALTER TABLE "cotacao_bid_frete_internacional"
      RENAME COLUMN "estado_provincia_destino_rodoviario_cotacao_bid_frete_internacional"
      TO "estado_provincia_destino_rodov_cotacao_bid_frete_internacional";
  END IF;
END $$;
...
SELECT public.bfi_reorder_table_columns(
  ...
  ARRAY[
    ...
    'estado_provincia_origem_rodov_cotacao_bid_frete_internacional',
    ...
    'estado_provincia_destino_rodov_cotacao_bid_frete_internacional',
