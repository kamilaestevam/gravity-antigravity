/**
 * dashboard-paineis.ts — Painéis de dashboard por usuário para o BID Frete Internacional
 *
 * GET    /api/v1/bid-frete-internacional/dashboard/paineis                                — lista painéis do usuário
 * POST   /api/v1/bid-frete-internacional/dashboard/paineis                                — cria novo painel
 * PUT    /api/v1/bid-frete-internacional/dashboard/paineis/reordenar                      — reordena painéis
 * PUT    /api/v1/bid-frete-internacional/dashboard/paineis/:id_painel_dashboard_pedido    — atualiza painel (patch)
 * DELETE /api/v1/bid-frete-internacional/dashboard/paineis/:id_painel_dashboard_pedido    — deleta painel
 */
  ...
  try {
    ...
    if (paineis.length === 0) {
      const padrao = await db.dashboardPainelUsuarioGlobal.create({
        data: {
          id_dashboard_painel_usuario_global: `bid_frete_${crypto.randomUUID()}`,
          id_organizacao:        idOrganizacao,
          id_usuario:            idUsuario,
...
dashboardPaineisRouter.put('/paineis/:id_painel_dashboard_pedido', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const updatedData = mapPatch(parsed.data)
