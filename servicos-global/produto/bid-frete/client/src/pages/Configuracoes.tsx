    ...
    <div ref={setNodeRef} style={style}>
      <div className={`cfg-card-row${!pref.visible ? ' cfg-card-row--oculto' : ''}${detalheAberto ? ' cfg-card-row--detalhe' : ''}`}>
        ...
        <div className="cfg-card-row__info">
          ...
          <div>
            <p className="cfg-card-row__nome">{def.labelKey}</p>
...
export default function Configuracoes() {
  ...
  const [statusList, setStatusList, , saveStatus, resetStatus, statusDirty] = useConfigState<PedidoStatusConfig[]>('status', [
    ...
    { id: 'em_cotacao', nome: 'EM_COTACAO', rotulo: 'Em cotação', cor: '#fbbf24', ordem: 2, is_sistema: true },
    { id: 'aprovada', nome: 'APROVADA', rotulo: 'Aprovada', cor: '#10b981', ordem: 3, is_sistema: false },
    { id: 'cancelada', nome: 'CANCELADA', rotulo: 'Cancelada', cor: '#ef4444', ordem: 4, is_sistema: false }
  ])
                  ...
                  {cardsPref.map((pref, i) => {
                    ...
                    return (
                      <div key={card.id} className={`cfg-kpi-preview-card ${!pref.visible ? 'cfg-kpi-preview-card--oculto' : ''}`}>
                        ...
                        <p className="cfg-kpi-preview-card__label">{card.labelKey}</p>
                      </div>
                    )
                  })}
                </div>
