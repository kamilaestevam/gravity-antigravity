...
export default function DetalheCotacao() {
  ...
  const tituloTopo = useMemo(() => {
    ...
    return {
      label:     t('bidfrete.detalhe_cotacao.titulo_cotacao', { numero: cotacao.numero_cotacao_bid_frete }),
      icone:     <FileText weight="duotone" size={22} />,
      subtitulo: cotacao.referencia_interna_cotacao_bid_frete
        ? t('bidfrete.detalhe_cotacao.ref_prefix', { ref: cotacao.referencia_interna_cotacao_bid_frete })
        : undefined,
    }
    ...
    <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', marginBottom: '1rem' }}>
      ...
      {cotacao.status_cotacao_bid_frete === 'RASCUNHO' && (
        <button
          ...
          type="button"
          onClick={async () => { await excluirCotacao(cotacao.id_cotacao_bid_frete); navigate('/bid-frete/cotacoes') }}
        >
