...
export default function Fornecedores() {
  ...
  const ratingsValidos = fornecedores.filter(f => f.rating_global != null)
  const ratingMedio = ratingsValidos.length > 0
    ? ratingsValidos.reduce((acc, f) => acc + (f.rating_global ?? 0), 0) / ratingsValidos.length
    : 0
  ...
  const colunas: TabelaGlobalColuna<Fornecedor>[] = [
    ...
    },
    {
      ...
      render: (val: TipoFornecedor) => {
        const cores = TIPO_FORNECEDOR_COLORS[val]
        return (
          <span style={{
            display: 'inline-flex',
            alignItems: 'center',
    ...
    },
    {
      ...
      render: (val: StatusFornecedor) => {
        const cores = STATUS_FORNECEDOR_COLORS[val]
        return (
          <span style={{
            display: 'inline-flex',
            alignItems: 'center',
