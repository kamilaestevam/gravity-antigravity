/**
 * Configuracoes.tsx — Configurações do SimulaCusto
 *
 * Categorias:
...
import { Kanban, CaretUp, CaretDown } from '@phosphor-icons/react'
import { TooltipGlobal } from '@nucleo/tooltip-global'
import {
  useCardPreferences, CARDS_CATALOGO,
  type CardPreferencia,
} from '../shared/preferencias-cards-estimativa-custo'
import { usePreferenciasKanbanEstimativaCusto } from '../shared/preferencias-kanban-estimativa-custo'
import { STATUS_LABELS } from '../shared/types'
import type { StatusEstimativaCusto } from '../shared/schemas-estimativa-custo'
import './configuracoes-estimativa-custo.css'
...
const PERIODO_KEY = 'sc:periodo-comparacao'

// ─── Mapa visual dos cards ────────────────────────────────────────────────────

const CARD_VISUAL: Record<string, { icone: React.ReactNode; cor: string }> = {
  total:                     { icone: <Calculator     weight="duotone" size={18} />, cor: 'var(--ws-accent, #818cf8)' },
  em_criacao:                { icone: <TrendUp        weight="duotone" size={18} />, cor: '#fbbf24' },
  criadas:                   { icone: <CheckCircle    weight="duotone" size={18} />, cor: '#34d399' },
  arquivadas:                { icone: <Archive        weight="duotone" size={18} />, cor: '#94a3b8' },
  custo_nacionalizado_medio: { icone: <CurrencyDollar weight="duotone" size={18} />, cor: '#34d399' },
  total_tributos_acumulado:  { icone: <Scales         weight="duotone" size={18} />, cor: '#f59e0b' },
}
...
const CATEGORIAS = [
  ...
  { id: 'kanban',       label: 'Kanban',       icone: <Kanban         size={15} weight="duotone" />, ativo: true  },
...
export default function Configuracoes() {
  ...
  const { prefs: prefsKanban, salvar: salvarPrefsKanban, alternarColuna } =
    usePreferenciasKanbanEstimativaCusto()

  function moverColunaKanban(status: StatusEstimativaCusto, direcao: -1 | 1) {
    const ordem = [...prefsKanban.ordem_colunas]
    const idx = ordem.indexOf(status)
    const destino = idx + direcao
    if (idx < 0 || destino < 0 || destino >= ordem.length) return
    ;[ordem[idx], ordem[destino]] = [ordem[destino], ordem[idx]]
    salvarPrefsKanban({ ...prefsKanban, ordem_colunas: ordem })
  }
          ...
          <div className="cfg-cards-wrapper">
            ...
            <section className="cfg-secao">
              <div className="cfg-secao__header">
                ...
                <TooltipGlobal descricao="Restaura os 4 cards padrão do produto">
            ...
            {disponiveis.length > 0 && (
              <section className="cfg-secao">
                <div>
                  ...
                  <p className="cfg-secao__desc">
                    Todas as métricas do SimulaCusto · clique em + para adicionar como card
                  </p>
                </div>
          ...
          <div className="cfg-cards-wrapper">
            <section className="cfg-secao">
              <div>
                ...
                <p className="cfg-secao__desc">
                  Os status são fixos do produto · use as setas para reordenar e o olho para ocultar colunas
                </p>
              </div>

              <div className="cfg-cards-lista">
                {prefsKanban.ordem_colunas.map((status, idx) => {
                  const oculta = prefsKanban.colunas_ocultas.includes(status)
                  return (
                    <div key={status} className={`cfg-card-row${oculta ? ' cfg-card-row--oculto' : ''}`}>
                      <div className="cfg-card-row__info">
                        <span className="cfg-card-row__icone" style={{ color: 'var(--ws-accent, #818cf8)' }}>
                          <Kanban weight="duotone" size={18} />
                        </span>
                        <div>
                          <p className="cfg-card-row__nome">{STATUS_LABELS[status]}</p>
                          <p className="cfg-card-row__desc">Coluna {idx + 1} do quadro</p>
                        </div>
                      </div>

                      <TooltipGlobal descricao="Mover para a esquerda">
                        <button
                          type="button"
                          className="cfg-eye-btn"
                          onClick={() => moverColunaKanban(status, -1)}
                          disabled={idx === 0}
                      ...
                      <TooltipGlobal descricao="Mover para a direita">
                        <button
                          type="button"
                          className="cfg-eye-btn"
                          onClick={() => moverColunaKanban(status, 1)}
                          disabled={idx === prefsKanban.ordem_colunas.length - 1}
                      ...
                      <TooltipGlobal descricao={oculta ? 'Exibir coluna' : 'Ocultar coluna'}>
                        <button
                          type="button"
                          className={`cfg-eye-btn${!oculta ? ' cfg-eye-btn--on' : ''}`}
                          onClick={() => alternarColuna(status)}
                          aria-label={oculta ? 'Exibir coluna' : 'Ocultar coluna'}
                        >
                          {oculta ? <EyeSlash size={15} weight="bold" /> : <Eye size={15} weight="bold" />}
                        </button>
                      </TooltipGlobal>
                    </div>
                  )
