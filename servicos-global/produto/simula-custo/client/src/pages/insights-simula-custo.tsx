/**
 * insights-simula-custo.tsx — Insights do Simula Custo.
 * Paridade visual com o Insights do Bid Frete Internacional (bfd-*):
 * KPIs com subtexto + tooltip, simulas recentes, distribuição por status.
...
import {
  obterKpisDashboardSimulaCusto,
  obterRecentesSimulaCusto,
} from '../shared/api'
import type { KpisDashboardSimulaCusto, SimulaCustoRecente } from '../shared/schemas-simula-custo'
...
import './insights-simula-custo.css'
...
export default function InsightsSimulaCusto() {
  const navigate = useNavigate()
  const { t } = useTranslation()

  const [kpis, setKpis] = useState<KpisDashboardSimulaCusto | null>(null)
  const [recentes, setRecentes] = useState<SimulaCustoRecente[]>([])
  const [carregando, setCarregando] = useState(true)

  useEffect(() => {
    let cancelado = false
    Promise.all([obterKpisDashboardSimulaCusto(), obterRecentesSimulaCusto()])
      .then(([k, r]) => {
        if (cancelado) return
        setKpis(k)
        setRecentes(r)
  ...
  const distribuicao = useMemo(() => {
    const total = kpis?.total_simulas ?? 0
  ...
  if (carregando) {
    return <ConteudoCarregandoSimulaCusto />
  }
  ...
  return (
    <div className="ecd-dashboard">
      ...
      <div className="ecd-kpi-grid">
        <CardBasicoGlobal
          titulo={t('simulacusto.insights.kpi_total', 'Total de Simulas')}
          icone={<Calculator weight="duotone" size={16} style={{ color: 'var(--ws-accent, #818cf8)' }} />}
          valor={String(kpis?.total_simulas ?? 0)}
          subtexto={subtextoTotal}
          variante="padrao"
          tooltip={
            <>
              <div className="cg-tooltip__row">
                <span>{t('simulacusto.simulas.em_criacao', 'Em Criação')}</span>
                <strong style={{ color: STATUS_COR.EM_CRIACAO }}>{kpis?.em_criacao ?? 0}</strong>
              </div>
              <div className="cg-tooltip__row">
                <span>{t('simulacusto.simulas.criadas', 'Criadas')}</span>
                <strong style={{ color: STATUS_COR.CRIADA }}>{kpis?.criadas ?? 0}</strong>
              </div>
              <div className="cg-tooltip__row">
                <span>{t('simulacusto.simulas.arquivadas', 'Arquivadas')}</span>
        ...
        <CardBasicoGlobal
          ...
          subtexto={t('simulacusto.insights.kpi.tributos.subtexto', {
            defaultValue: 'II + IPI + PIS + COFINS + ICMS das simulas calculadas',
          })}
          variante="padrao"
          tooltip={
            <div className="cg-tooltip__row">
              <span>{t('simulacusto.insights.kpi.tributos.tooltip', 'Soma dos tributos de todas as simulas')}</span>
        ...
        <div className="ecd-card ecd-card--accent-indigo ecd-recentes">
          <div className="ecd-card__head">
            <div className="cg-card__header">
              ...
              <p className="cg-card__label">
                {t('simulacusto.insights.recentes', 'Simulas recentes')}
              </p>
            </div>
            <button
              type="button"
          ...
          {recentes.length === 0 ? (
            <div className="ecd-vazio">
              {t('simulacusto.simulas.vazio', 'Nenhuma simula encontrada')}
            </div>
          ) : (
            <ul className="ecd-recentes-lista">
              {recentes.map(e => (
                <li key={e.id_simula_custo}>
                  <button
                    type="button"
                    className="ecd-recente"
                    onClick={() => navigate(rotaDetalheSimulaCusto(e.id_simula_custo))}
                  >
                    <span className="ecd-recente__numero">{e.numero_simula_custo}</span>
                    <span className="ecd-recente__ncm">NCM {e.ncm_simula_custo}</span>
                    <span className={`sc-est-badge sc-est-badge--${STATUS_BADGE[e.status_simula_custo]}`}>
                      {STATUS_LABELS[e.status_simula_custo]}
                    </span>
                    <span className="ecd-recente__valor">{brl(e.custo_nacionalizado_brl_simula_custo)}</span>
                    <span className="ecd-recente__data">{dataBR(e.data_criacao_simula_custo)}</span>
                  </button>
                </li>
              ))}
            </ul>
