/**
 * ICMS na importação — cálculo "por dentro" (LC 87/1996, RICMS, Convênio ICMS 38/2013).
 *
 * Base legal do gross-up: soma das parcelas anteriores ÷ (1 − alíquota interna da UF).
 * Redução (TTD/convênio): a alíquota interna permanece no divisor; o imposto devido
 * incide com a alíquota efetiva (interna × (1 − % redução da base)).
 */
import type { ModalidadeRecolhimentoIcmsSimulaCusto } from './modalidade-recolhimento-icms-simula-custo.js'
import { icmsCompoeNacionalizadoSimulaCusto } from './modalidade-recolhimento-icms-simula-custo.js'
...
export interface ParametrosCalculoIcmsImportacaoSimulaCusto {
  ...
  /** Alíquota interna da UF — gross-up "por dentro". */
  aliquotaInternaUf: number
  /** Alíquota efetiva informada (modalidade REDUCAO) ou igual à interna (INTEGRAL). */
...
export function calcularIcmsImportacaoSimulaCusto(
  ...
  if (modalidade === 'INTEGRAL') {
    ...
    return {
      ...
      reducao_base: null,
      base_calculo: base,
      valor: icmsCompoeNacionalizadoSimulaCusto(modalidade) ? valorBruto : 0,
    }
  }

  // REDUCAO — gross-up com alíquota interna; imposto com alíquota efetiva (≤ interna).
