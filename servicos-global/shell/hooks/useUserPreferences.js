 ...
 */
async function fetchPreferencias(userId, tenantId) {
    try {
 ...
 */
async function savePreferencias(userId, tenantId, payload) {
    try {
 ...
 */
export function useUserPreferences({ userId, tenantId }) {
    ...
    useEffect(() => {
        ...
        fetchPreferencias(userId, tenantId).then((prefs) => {
            if (!prefs)
                return; // Falhou — mantém estado do localStorage
            ...
            if (prefs.tooltips_disabled !== undefined &&
            ...
            if (prefs.sidebar_open !== undefined &&
