import { PrismaClient, ActorType, AlertStatus } from '../../../generated/index.js'
...
export const AlertEngine = {
  ...
  async evaluateRule(
    rule: Awaited<ReturnType<typeof prisma.alertRule.findFirst>> & object,
    log: AuditLogInput,
    logId: string
  ): Promise<void> {
    if (!rule) return
    ...
    if (rule.threshold_count && rule.threshold_window_seconds) {
      ...
      const alertEvent = await prisma.alertEvent.create({
        data: {
          tenant_id: log.tenant_id,
          rule_id: rule.id,
          actor_type: log.actor_type as ActorType,
          actor_id: log.actor_id,
