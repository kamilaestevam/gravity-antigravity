...
>
> ⚠️ **REGRA ABSOLUTA:** Ver [Lint Tenant-Safety](../../governanca/convencao-tecnica/lint-tenant-safety/SKILL.md) — **REGRA 4** do linter: toda chave de cache **deve** começar com `tenant:` (ou `tenant:_global:`). Esta skill mostra como o padrão é aplicado, mas a regra é validada em CI.
...
**Quando usar:** dados globais (sem `id_organizacao` na chave), instância única, < 1000 entries. **Não** compartilhado entre réplicas — cada instância tem o seu Map.
...
export async function redisInvalidate(pattern: string): Promise<number> {
  ...
  for await (const keys of stream) {
    if (keys.length > 0) {
      ...
      removed += await redis.unlink(...keys as string[])
    }
  }
  return removed
}
...
Toda chave de cache **deve** começar com `tenant:` ou `tenant:_global:`. O CI bloqueia o deploy de qualquer chamada `redis.set/get/del/expire` que não respeite esse prefixo.

```typescript
// ✅ correto — prefixo tenant:<idOrganizacao> isola o cache por organização
const cacheKey = `tenant:${idOrganizacao}:dashboard:kpis`
const cacheKey = `tenant:${idOrganizacao}:produtos:${idProduto}`

// ✅ correto — _global: para dados sem organização (ex: tabela NCM)
const cacheKey = `tenant:_global:ncm:8471.30`  // global exige justificativa em comment

// ❌ ERRO — sem prefixo tenant:
const cacheKey = `dashboard:kpis:${idOrganizacao}`     // bloqueado em CI

// ❌ ERRO — sem nenhum identificador de organização
const cacheKey = `dashboard:kpis`                       // bloqueado em CI
```

> **Helper futuro (Sprint 2 — ver [SDK Resolvedor de Organização](../../governanca/lei/sdk-resolvedor-organizacao/SKILL.md)):** `tenantCache(req.organizacao)` aplicará o prefixo automaticamente. Enquanto não chega, escreva o prefixo `tenant:` na mão.
...
| Alíquotas fiscais (NCM) | 24 horas | Redis (`tenant:_global:`) | Ao atualizar base |
| PTAX (câmbio) | 1 hora | Redis (`tenant:_global:`) | Ao atualizar cotação |
...
async function getDashboardKpis(idOrganizacao: string): Promise<KPIs> {
  ...
  const kpis = await calcularKpis(idOrganizacao)
...
async function createCotacao(input: CotacaoInput) {
  ...
  // Invalidar caches relacionados — chaves seguem o prefixo tenant:
  await redisInvalidate(`tenant:${input.id_organizacao}:dashboard:kpis`)
  await redisInvalidate(`tenant:${input.id_organizacao}:cotacoes:count`)
...
- **hit rate** por prefixo de chave (`tenant:*:dashboard:*`, etc.)
...
- [ ] Chave começa com `tenant:${idOrganizacao}:` (ou `tenant:_global:` justificado)?
