...
> 📋 **IDs técnicos visíveis na UI (decisão dono 2026-06-12 — PR #303).** Todas as listagens do Configurador/Admin expõem os IDs técnicos em colunas/cards copiáveis: `id_organizacao` + `suid_empresa_organizacao` (Admin > Organizações e card HQ da Visão Geral), `id_workspace` (Workspaces), `id_usuario` (Usuários e Admin > Usuários Globais), `id_fornecedor` (Fornecedores e Admin > Fornecedores). **Padrão único:** componente `src/shared/CelulaIdCopiavel.tsx` (monospace, clique copia, toast) — colunas de ID novas DEVEM usá-lo, nunca recriar o snippet inline. Org sem Empresa vinculada → badge âmbar "sem empresa" (Mand. 08 — ausência é informação; é a causa do erro de onboarding incompleto no Novo Pedido). Payloads: `GET /v1/admin/organizacoes` e `GET/PUT /v1/admin/visao-geral` incluem `suid_empresa_organizacao`; o contrato da Visão Geral é Zod (`platformConfigApiSchema` em `api-client.ts`).
...
## HUB — puzzle Seus Produtos Gravity (PR #404)

Faixa superior de `/hub` (`SelecionarWorkspace`) alinhada à Gravity Store.

| Peça | Caminho |
|------|---------|
| Página | `src/pages/SelecionarWorkspace.tsx` |
| CSS | `src/pages/hub-unificado.css` |
| Grade + cabeçalho | `src/components/grade-produtos-contratados-hub.tsx` |
