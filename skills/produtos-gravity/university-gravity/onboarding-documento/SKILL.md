...
3. **Espaço entre parágrafos:** **12px** (`MANUAL_ESPACO_PARAGRAFO_PX`) via `manualMargemParagrafo(i, total)`; **0** no último parágrafo de cada bloco — ver ONBOARDING-DOCUMENTO.md **§9.1.1** e `manual-tipografia.ts`.
4. **Alinhamento:** corpo narrativo **justificado** (`MANUAL_ALINHAMENTO_CORPO`) em parágrafos e callouts — ver **§9.1.2**; títulos e rótulos ficam à esquerda.
5. **Passos:** rótulo `PASSO NN` em `12px` `#818cf8`; título do passo em `0.92rem` 100%; parágrafos via `ManualParagrafo`.
6. **URLs:** sempre `https://usegravity.com.br/...` no texto; login canônico = `https://usegravity.com.br/login`.
7. **Ícones:** token `{{icone:slug}}` **com** escrita descritiva no mesmo parágrafo (ex.: “ícone de olho {{icone:olho}}”).
8. **Screenshots:** salvar em `public/university/screenshots/` e referenciar caminho absoluto `/university/screenshots/...`.
9. **Intro de fluxo:** um parágrafo resumido na seção; detalhes nos `passosVisuais`.
