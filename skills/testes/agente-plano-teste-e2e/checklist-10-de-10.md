...
- Testar inputs adversariais: `<script>alert(1)</script>`, `' OR 1=1--`, string de 10.000 caracteres → sistema deve rejeitar sem crash e sem stack trace exposto
...
- (criticidade alta+) Abrir 2 contextos Playwright simultâneos editando o mesmo registro → verificar que o sistema resolve o conflito sem corromper dados (race condition)
- (criticidade alta+) Expirar JWT no meio do preenchimento de formulário → verificar redirect para login sem perda de contexto e sem tela branca
...
- (criticidade alta+) Contexto A salva dado → Contexto B (aberto antes do save) recarrega → verificar que B vê dado atualizado, não versão em cache (cache invalidation entre sessões)
