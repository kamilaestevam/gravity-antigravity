...
const InteracaoSchema = z.discriminatedUnion('tipo', [
  ...
  z.object({ tipo: z.literal('multiContexto'), descricao: z.string() }),  // 2 contextos simultâneos (race condition)
])
...
export const PlanoTesteSchema = z.object({
  ...
  downstream:     z.string().optional(),           // onde o dado aparece após save; "nenhum" se ação local
  smeRevisadoPor: z.string().nullable().default(null),
  smeRevisadoEm:  z.string().nullable().default(null),  // ISO timestamp
