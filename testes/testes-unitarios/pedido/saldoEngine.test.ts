...
describe('saldoEngine.transferir', () => {
  it('deve transferir quantidade e debitar saldo atual', async () => {
    const item = criarItemMock({ quantidade_atual: 1000, quantidade_transferida: 0 })
    ...
    expect(result.quantidade_atual).toBe(600)
    expect(result.quantidade_transferida).toBe(400)
    expect(result.quantidade_inicial).toBe(1000)
  })

  it('deve transferir toda a quantidade disponivel', async () => {
    const item = criarItemMock({ quantidade_atual: 500, quantidade_transferida: 500 })
    const prisma = criarPrismaMock(item)
    ...
    expect(result.quantidade_atual).toBe(0)
    expect(result.quantidade_transferida).toBe(1000)
  })

  it('deve REJEITAR transferencia quando saldo insuficiente (anti-sobre-execucao)', async () => {
    const item = criarItemMock({ quantidade_atual: 100, quantidade_transferida: 900 })
    const prisma = criarPrismaMock(item)
...
describe('saldoEngine.cancelar', () => {
  it('deve cancelar quantidade e debitar saldo atual', async () => {
    const item = criarItemMock({ quantidade_atual: 1000, quantidade_cancelada: 0 })
    const prisma = criarPrismaMock(item)
    ...
    expect(result.quantidade_atual).toBe(800)
    expect(result.quantidade_cancelada).toBe(200)
    expect(result.quantidade_inicial).toBe(1000)
  })

  it('deve REJEITAR cancelamento quando saldo insuficiente', async () => {
    const item = criarItemMock({ quantidade_atual: 50, quantidade_transferida: 950 })
    const prisma = criarPrismaMock(item)
...
describe('saldoEngine.atualizarPronta', () => {
  it('deve atualizar quantidade pronta sem afetar saldo', async () => {
    const item = criarItemMock({ quantidade_atual: 1000, quantidade_pronta: 0 })
    const prisma = criarPrismaMock(item)
    ...
    expect(result.quantidade_pronta).toBe(600)
    expect(result.quantidade_atual).toBe(1000) // nao afeta saldo
    expect(result.quantidade_transferida).toBe(0)
  })
...
describe('saldoEngine.validarIntegridade', () => {
  ...
  it('deve detectar integridade violada (soma nao bate)', () => {
    const result = saldoEngine.validarIntegridade({
      ...
      quantidade_atual: 600,
      quantidade_transferida: 200,
      quantidade_cancelada: 100, // 600 + 200 + 100 = 900 != 1000
      quantidade_pronta: 0,
    })
  ...
  it('deve tolerar diferenca de arredondamento (<0.001)', () => {
    const result = saldoEngine.validarIntegridade({
      ...
      quantidade_atual: 33.333,
      quantidade_transferida: 33.333,
      quantidade_cancelada: 33.334,
      quantidade_pronta: 0,
    })
    expect(result).toBe(true)
  })
